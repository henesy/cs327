...
/* compare turn priority */
int compare_turn(const void *key, const void *with) {
	if(((Sprite *) key)->t < ((Sprite *) with)->t)
		return -1;
	if(((Sprite *) key)->t > ((Sprite *) with)->t)
		return 1;
	if(((Sprite *) key)->t == ((Sprite *) with)->t) {
		if(((Sprite *) key)->sn < ((Sprite *) with)->sn)
			return -1;
		if(((Sprite *) key)->sn > ((Sprite *) with)->sn)
			return 1;
	}
...
void copy_sprite(Sprite * a, Sprite * b) {
	a->p.x  = b->p.x;
	a->p.y  = b->p.y;
	a->c    = b->c;
	a->s.in = b->s.in;
